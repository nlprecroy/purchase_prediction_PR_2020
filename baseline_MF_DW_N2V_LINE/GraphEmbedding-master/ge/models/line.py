...
class LINE:
    def __init__(self, graph, embedding_size=8, negative_ratio=100, order='second', if_float = False):
        """
    ...
    def batch_iter(self, node2idx):
        ...
        while True:
            if mod == 0:
                ...
                sign = []
                for head, tail in zip(h, t):
                    sign.append(self.graph[head][tail].get('weight', 1.0))
                sign = np.array(sign)
                print("sign >>>>>>>>>", sign)
    ...
    def train(self, batch_size=1024, epochs=1, initial_epoch=0, verbose=2, times=1):
